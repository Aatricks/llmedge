    ...
    * is reused for both prompt and generation phases.
     ...
     */
        data class InferenceParams(
            ...
            /**
             * Micro-batch size for prompt processing (llama.cpp n_ubatch). Larger values speed up
    ...
    /**
     * Returns true when the last completion ended because the context window filled up
     * mid-generation. The partial response is still returned/streamed in that case —
     * this flag lets callers distinguish a truncated answer from a natural end.
     */
    fun wasContextLimitReached(): Boolean = SmolLMFacadeOperations.wasContextLimitReached(this)
    ...
    /**
     * Guards native calls against a concurrent [close]: every completion step and
     * [close] itself take this lock and re-read the handle inside it.
     */
    internal val nativeCallLock = Any()

    /**
     * Re-applies the P-core affinity mask from the thread that is about to run
     * inference. The mask is recorded at load time, but load runs on a different
     * dispatcher than generation, and the native pinning only affects the calling
     * thread (and its OpenMP team) — so it must be applied from the inference thread.
     */
    internal fun applyInferenceThreadAffinity() {
        val mask = runtimeState.pCoreMask
        if (mask == 0L) return
        synchronized(nativeCallLock) {
            val nativePtr = runtimeState.nativePtr
            if (nativePtr != 0L) {
                setThreadAffinity(nativePtr, mask)
            }
        }
    }
    ...
    @JvmName("wasContextLimitReached")
    internal external fun wasContextLimitReached(modelPtr: Long): Boolean
